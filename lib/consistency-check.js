    ...
    if (r.paymentReceived?.partial) {
      ...
      add(
        ...
        r.paymentReceived.method === "batch"
          ? `Part of a combined payment of ${euro(r.paymentReceived.amount)} from ${r.paymentReceived.contact} on ${r.paymentReceived.date}, which pays exactly: ${r.paymentReceived.batchWith.join(" + ")} — link in Yuki.`
