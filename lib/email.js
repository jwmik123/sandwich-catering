      ...
      <body>
        <div class="container">
          ...
          <div class="header">
            ...
            <div class="order-id">Payment Status: ${orderDetails.paymentMethod === "invoice" ? (orderDetails.paymentStatus === "paid" ? "Paid" : "Pending") : "Paid"}</div>
          ...
          <div class="price-section">
            <div class="price-row">Subtotal: €${calculateSubtotal(orderDetails).toFixed(2)}</div>
            ${orderDetails.deliveryCost > 0 ? `<div class="price-row">Delivery: €${(orderDetails.deliveryCost || 0).toFixed(2)}</div>` : '<div class="price-row">Delivery: Free</div>'}
...
function calculateTotal(orderDetails) {
  ...
  if (orderDetails.selectionType === "custom") {
    ...
    subtotal = orderDetails.totalSandwiches * 6.38; // Variety selection price per sandwich
  }

  // Add delivery cost if present
  const deliveryCost = orderDetails.deliveryCost || 0;
  return subtotal + deliveryCost; // Total excluding VAT
}
