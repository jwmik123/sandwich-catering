      ...
      <body>
        <div class="container">
          ...
          <div class="header">
            ...
            <div class="order-id">Payment Status: ${orderDetails.paymentStatus === "paid" ? "Paid" : "Pending"}</div>
...
export async function sendOrderConfirmation(order, isInvoiceEmail = false) {
  ...
  try {
    ...
    // Generate PDF only for direct payments or invoice emails
    let pdfBuffer;
    if (!isInvoiceEmail) {
      try {
        // Generate PDF for direct payments
        ...
        // Continue without PDF for direct payments
      }
    }
    ...
    const emailData = {
      ...
      subject: EMAIL_CONSTANTS.EMAIL_SUBJECTS.ORDER_CONFIRMATION(order.quoteId),
      html: emailHtml,
    };

    // Add PDF attachment only for direct payments or invoice emails
    if (pdfBuffer) {
