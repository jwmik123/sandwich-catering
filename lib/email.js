...
export async function sendInvoiceEmail(order) {
  ...
  try {
    ...
    // Ensure we have a fixed, valid from address
    const fromEmail = "LunchCatering <orders@thesandwichbar.nl>";
    ...
    console.log("Generating PDF with safe data...");
    try {
      const pdfBuffer = await renderToBuffer(
        <InvoicePDF
      ...
      console.log("- Subject:", `Invoice for Order ${safeOrderData.quoteId}`);

      const { data, error } = await resend.emails.send({
        from: fromEmail,
        to: [order.email],
        subject: `Invoice for Order ${safeOrderData.quoteId}`,
