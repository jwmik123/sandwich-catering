...
export async function buildInvoiceOverview() {
  ...
  // Open items in Yuki that are not CAT- invoices of ours (pre-numbering
  // catering invoices, manual ones). A combined payment can cover those too.
  const catRefs = new Set(invoices.map((i) => String(i.invoiceNumber || "").trim()));
  const otherOpenItems = [...openMap.entries()]
    .filter(([ref]) => !catRefs.has(ref))
    .map(([ref, y]) => ({
      reference: ref,
      customer: y.contact,
      openAmount: y.openAmount,
      date: y.date,
    }));
