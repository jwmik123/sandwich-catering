...
function createOrderSmsContent(orderData) {
  ...
  const street = deliveryDetails.address?.street || "";
  const houseNumber = deliveryDetails.address?.houseNumber || "";
  const houseNumberAddition = deliveryDetails.address?.houseNumberAddition || "";
  const postalCode = deliveryDetails.address?.postalCode || "";
  const city = deliveryDetails.address?.city || "";

  // Format full address
  const fullAddress = `${street} ${houseNumber}${houseNumberAddition}, ${postalCode} ${city}`;
  ...
  // Format "How did you find us?" information
  const formatHowDidYouFindUs = () => {
    ...
    let sources = howDidYouFindUs.map(source => labels[source] || source).join(", ");
    
    // Add "other" details if specified
    if (howDidYouFindUs.includes("other") && howDidYouFindUsOther.trim()) {
      sources = sources.replace("Other", `Other: ${howDidYouFindUsOther.trim()}`);
    }

    return `Gevonden via: ${sources}`;
  };

  const howFoundUsText = formatHowDidYouFindUs();
...
Adres: ${fullAddress}
${customerInfo}
Contact: ${deliveryDetails.phoneNumber || "Niet opgegeven"}${howFoundUsText ? `\n${howFoundUsText}` : ""}
