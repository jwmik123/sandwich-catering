...
export class YukiApiClient {
   ...
   * @param {number} invoiceAmount - Total invoice amount for validation (optional but recommended)
  ...
  formatInvoiceFromOrderData(
    ...
    const addLine = (sandwichId, selection) => {
      ...
      // Convert VAT-inclusive price to VAT-exclusive for Yuki (Yuki adds 9% VAT itself)
      // Round at each step to ensure consistent 2-decimal precision
      const unitPriceInclVAT = round2(selection.subTotal / selection.quantity);
      const unitPrice = round2(unitPriceInclVAT / 1.09); // Convert to VAT-exclusive
    ...
    if (orderData.selectionType === "custom") {
      ...
      varietyTypes.forEach((type) => {
        ...
        if (quantity > 0) {
          // Use €6.83 as VAT-exclusive price, add gluten-free surcharge if applicable
          // Round to ensure consistent 2-decimal precision
          const unitPrice = round2(type.key === "glutenFree" ? 6.83 + GLUTEN_FREE_SURCHARGE : 6.83);
          lines.push({
      ...
      if (orderData.upsellAddons && orderData.upsellAddons.length > 0) {
        orderData.upsellAddons.forEach(addon => {
          ...
          // addon.subTotal is VAT-inclusive, convert to VAT-exclusive unit price
          // Round at each step to ensure consistent 2-decimal precision
          const unitPriceInclVAT = round2((addon.subTotal || 0) / (addon.quantity || 1));
          const unitPrice = round2(unitPriceInclVAT / 1.09);
    ...
    // Validate line items against stored invoice amount (if provided)
    // This ensures Yuki invoice matches the Sanity invoice
    if (invoiceAmount && typeof invoiceAmount === 'number') {
      // Calculate what Yuki will compute from our line items
      // Use round2 for consistent rounding with the rest of the app
      const calculatedSubtotal = lines.reduce((sum, line) =>
        round2(sum + round2(line.quantity * line.unitPrice)), 0
      );
      const calculatedVAT = round2(calculatedSubtotal * 0.09);
      const calculatedTotal = round2(calculatedSubtotal + calculatedVAT);

      console.log('=== YUKI INVOICE VALIDATION ===');
      console.log(`Stored invoice total: €${invoiceAmount.toFixed(2)}`);
      console.log(`Calculated from line items: €${calculatedTotal.toFixed(2)}`);
      console.log(`Difference: €${Math.abs(calculatedTotal - invoiceAmount).toFixed(2)}`);

      // If there's a rounding discrepancy > €0.02, add adjustment line
      const difference = invoiceAmount - calculatedTotal;
      if (Math.abs(difference) > 0.02) {
        console.warn(`⚠️  Adding rounding adjustment of €${difference.toFixed(2)}`);
        lines.push({
          description: "Afrondingsverschil",
          quantity: 1,
          unitPrice: round2(difference / 1.09), // Convert to VAT-exclusive
          glAccount: "80001",
        });
      } else if (Math.abs(difference) > 0.00) {
        console.log(`✓ Minor rounding difference of €${difference.toFixed(2)} is acceptable`);
      } else {
        console.log('✓ Invoice amounts match perfectly');
      }
    }
