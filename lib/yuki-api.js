...
export class YukiApiClient {
  ...
  async createSalesInvoice(invoiceData) {
    ...
    // Calculate total VAT amount (9% on all line items)
    const totalNet = invoiceData.lines.reduce(
      (sum, line) => sum + line.unitPrice * line.quantity,
      0
    );
    const vatAmount = totalNet * 0.09;
    ...
    const vatLine = `
      <InvoiceLine>
        ...
        <Product>
          <Description>BTW 9%</Description>
          <Reference>VAT-9</Reference>
          <Category xsi:nil="true" />
          <SalesPrice>${vatAmount.toFixed(2)}</SalesPrice>
          <VATPercentage>0.00</VATPercentage>
          <VATIncluded>false</VATIncluded>
          <VATType>1</VATType>
          <GLAccountCode>1520</GLAccountCode>
          <Remarks />
        </Product>
      </InvoiceLine>
    `;
  ...
  formatInvoiceFromOrderData(
      ...
      varietyTypes.forEach((type) => {
        ...
        if (quantity > 0) {
          lines.push({
            ...
            quantity,
            unitPrice: 6.38, // VAT-exclusive price (€6.38 incl. VAT = €5.92 excl. VAT at 9%)
            glAccount: "8000",
          });
        }
      });
    ...
    // Calculate due date as 14 days after delivery date (not current date)
    let dueDate;
