...
export class YukiApiClient {
  ...
  formatInvoiceFromOrderData(
    ...
    if (orderData.selectionType === "custom") {
      ...
      varietyTypes.forEach((type) => {
        ...
        if (quantity > 0) {
          ...
          const unitPrice = type.key === "glutenFree" ? 6.83 + GLUTEN_FREE_SURCHARGE : 6.83;
          lines.push({
    ...
    if (orderData.drinksWithDetails && orderData.drinksWithDetails.length > 0) {
      orderData.drinksWithDetails.forEach(drink => {
        lines.push({
          ...
          unitPrice: drink.price, // Already VAT-exclusive from Sanity
          glAccount: "80001", // Sales account
        });
      });
    }
    ...
    if (invoiceAmount && typeof invoiceAmount === 'number') {
      ...
      const calculatedVAT = Math.ceil(calculatedSubtotal * 0.09 * 100) / 100;
      ...
      if (Math.abs(difference) > 0.02) {
        ...
        lines.push({
          ...
          quantity: 1,
          unitPrice: difference / 1.09, // Convert to VAT-exclusive
          glAccount: "80001",
        });
