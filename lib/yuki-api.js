...
export class YukiApiClient {
  ...
  formatInvoiceFromOrderData(
    ...
    if (orderData.selectionType === "custom") {
      // Process custom selections - use VAT-inclusive prices directly
      ...
      // Process variety selection - use VAT-inclusive price directly
      const varietyTypes = [
      ...
      varietyTypes.forEach((type) => {
        ...
        if (quantity > 0) {
          // Use €6.38 directly (VAT-inclusive price)
          lines.push({
            description: `${type.name} Sandwiches`,
            quantity,
            unitPrice: 6.38, // VAT-inclusive price
            glAccount: "8000",
          });
        }
      });
