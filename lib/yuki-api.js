...
/**
 * Finds the Yuki contact code this customer already has, so every invoice for
 * the same customer lands on one contact. In order:
 *   1. the code this invoice was booked with before (a re-booking)
 *   2. the oldest code used on an earlier invoice for the same VAT number,
 *      invoice e-mail address or company name
 *   3. an existing Yuki contact with exactly this name that has a code
 *   4. a new code
 * Only contacts with a code can be referenced from a sales invoice — Yuki's
 * schema has no contact id — so contacts the bookkeeper created without one
 * are not reused (yet).
 * @returns {Promise<{code: string, source: string}>}
 */
export async function resolveYukiContactCode(invoiceDoc, yukiClient) {
  if (invoiceDoc.yukiContactCode) {
  ...
  const previous = await client.fetch(
    `*[_type == "invoice" && defined(yukiContactCode) && yukiSent == true && _id != $id]{
  ...
  if (wantName) {
    try {
      const found = await yukiClient.searchContactsByName(name);
      const exact = found.find(
        (c) => c.code && normalizeCustomerName(c.name) === wantName
      );
      if (exact) return { code: exact.code, source: "existing Yuki contact" };
    } catch (e) {
